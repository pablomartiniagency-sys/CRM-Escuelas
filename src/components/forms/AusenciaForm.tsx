    ...
    <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
      <div className="space-y-4">
        <div>
          ...
          <select {...register("alumno_id")} disabled={!!defaultAlumnoId}
            ...
            {alumnos.map((a: { alumno_id: string; nombre_completo: string }) => (
              <option key={a.alumno_id} value={a.alumno_id}>{a.nombre_completo}</option>
            ))}
          </select>
          {errors.alumno_id && <p className="text-xs text-red-500 mt-1">{errors.alumno_id.message}</p>}
        </div>
        ...
        <div>
          ...
          <input {...register("motivo")} placeholder="Enfermedad, cita medica..."
            className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Como informo la familia</label>
          <select {...register("canal_comunicacion")}
            className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div className="flex items-center gap-2">
          <input type="checkbox" id="justificada_check" {...register("justificada")} className="rounded" />
          <label htmlFor="justificada_check" className="text-sm text-gray-700">Ausencia justificada</label>
        </div>
      </div>
