      ...
      {!preview ? (
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input {...register("titulo")} placeholder="Circular de inicio de curso"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Canal de envio</label>
              <select {...register("canal_envio")}
                className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Centro destinatario</label>
            ...
            {(canalValue === "email" || canalValue === "ambos") && (
              <div className="col-span-2">
                ...
                <input {...register("asunto_email")} placeholder="Asunto del correo"
                  className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            )}
            <div className="col-span-2">
              ...
              {errors.cuerpo && <p className="text-xs text-red-500 mt-1">{errors.cuerpo.message}</p>}
              <p className="text-xs text-gray-400 mt-1 text-right">{cuerpoValue?.length ?? 0} caracteres</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setPreview(false)} className="text-xs text-blue-600 hover:text-blue-700">Editar</button>
          </div>
            ...
            <div className="max-w-sm mx-auto">
              ...
              <div className="bg-gray-100 rounded-b-xl p-4 min-h-[180px]">
                <div className="bg-white rounded-[18px] rounded-tl-none p-3 shadow-sm max-w-[85%] ml-2">
                  ...
                  <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{captured?.cuerpo}</p>
          ...
          {(captured?.canal_envio === "email" || captured?.canal_envio === "ambos") && (
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <p className="text-xs text-gray-500"><span className="font-medium">Asunto:</span> {captured?.asunto_email ?? captured?.titulo}</p>
                <p className="text-xs text-gray-500 mt-0.5"><span className="font-medium">Para:</span> {captured?.destinatarios}</p>
              </div>
              <div className="p-5">
                <h2 className="text-lg font-bold text-gray-900 mb-3">{captured?.titulo}</h2>
                <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{captured?.cuerpo}</p>
