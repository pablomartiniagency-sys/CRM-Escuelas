            ...
            <div className="mx-6 mt-4 p-3 bg-orange-50 border border-orange-200 rounded-xl flex items-start gap-2.5">
              ...
              <div>
                <p className="text-xs font-semibold text-orange-800 mb-0.5">
                  Informacion medica relevante
                </p>
                {alumno.alergias && (
                  <p className="text-xs text-orange-700">Alergias: {alumno.alergias}</p>
                )}
                {alumno.notas_medicas && (
                  <p className="text-xs text-orange-700 mt-0.5">{alumno.notas_medicas}</p>
                )}
              </div>
            </div>
          )}
                ...
                {[
                  ...
                  { label: "Edad", value: edad !== null ? `${edad} anos` : undefined },
                  {
                    ...
                    value: alumno.fecha_nacimiento
                      ? format(parseISO(alumno.fecha_nacimiento), "d 'de' MMMM 'de' yyyy", {
                          locale: es,
                        })
                      : undefined,
                  },
          ...
          {tab === "familia" && (
            <div className="p-6 space-y-3">
              <p className="text-sm font-medium text-gray-700">
                {familias.length} contactos familiares
              </p>
              {loadingF ? (
                ...
                <div className="space-y-2.5">
                  {familias.map(
                    (f: {
                      ...
                      <div
                        key={f.id ?? f.contactos?.contacto_id}
                        className="p-4 bg-white border border-gray-200 rounded-xl"
                      >
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div>
                            <p className="text-sm font-semibold text-gray-900">
                              {f.contactos?.nombre_completo}
                            </p>
                          ...
                          {[
                            {
                              ...
                              value: f.es_responsable_legal,
                              icon: Shield,
                            },
                            {
                              label: "Autorizado recogida",
                              value: f.autorizado_recogida,
                              icon: CheckCircle,
                            },
                            {
                              label: "Contacto emergencia",
                              value: f.es_contacto_emergencia,
                              icon: AlertTriangle,
                            },
                          ].map(({ label, value, icon: Icon }) => (
                            <div
                              key={label}
                              className={`flex items-center gap-1.5 px-2 py-1 rounded-md ${value ? "bg-emerald-50 text-emerald-700" : "bg-gray-50 text-gray-400"}`}
                            >
                              <Icon size={11} />
                              <span>{label}</span>
                            </div>
            ...
            <div className="p-6 space-y-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-gray-700">
                  ...
                  {ausencias.filter((a: { justificada?: boolean }) => !a.justificada).length} sin
                  justificar
                </span>
                {!showAusenciaForm && (
                  <button
              ...
              {showAusenciaForm && (
                <div className="border border-blue-200 rounded-xl bg-blue-50/30">
                  ...
                  <AusenciaForm
                    ...
                    onSuccess={() => setShowAusenciaForm(false)}
                  />
                </div>
              )}
              {loadingAU ? (
                ...
                <div className="space-y-2">
                  {ausencias.map(
                    (au: {
                      ...
                      >
                        <div
                          className={`w-1 h-10 rounded-full flex-shrink-0 ${au.justificada ? "bg-emerald-400" : "bg-orange-400"}`}
                        />
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400">
                            {au.motivo ?? "Sin motivo especificado"}
                          </p>
                        </div>
