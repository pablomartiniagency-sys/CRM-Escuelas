                  ...
                  {evs.map((ev: Evento, idx: number) => {
                    ...
                    return (
                      ...
                      >
                        {/* Fecha */}
                        <div
                          className={`flex-shrink-0 w-12 h-12 rounded-xl flex flex-col items-center justify-center border ${esHoy ? "bg-blue-600 border-blue-600 text-white" : "bg-gray-50 border-gray-200 text-gray-700"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {ev.tipo && (
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded-full font-medium border flex-shrink-0 ${colorClass}`}
                              >
                                {ev.tipo}
                              </span>
                            )}
                          ...
                          <div className="flex items-center gap-3 text-xs text-gray-400">
                            {ev.clientes && (
                              <span className="flex items-center gap-1">
                                <Building2 size={11} />
                                {(ev.clientes as { nombre: string }).nombre}
                              </span>
                            )}
                            {ev.aula && (
                              <span className="flex items-center gap-1">
                            ...
                            {ev.fecha_fin && (
                              <span>
                                hasta {format(parseISO(ev.fecha_fin), "d MMM", { locale: es })}
                              </span>
                            )}
                          </div>
                        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            {["Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"].map((d) => (
              <div
                key={d}
                className="py-2.5 text-center text-xs font-semibold text-gray-400 border-b border-gray-100"
              >
                {d}
              </div>
            ))}
        ...
        {selectedEvento && (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
              <div
                className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center border flex-shrink-0 ${TIPO_COLORS[selectedEvento.tipo ?? ""] ?? TIPO_COLORS.Otro}`}
              >
                <span className="text-[10px] font-bold uppercase">
                  {format(parseISO(selectedEvento.fecha_inicio), "MMM", { locale: es })}
                </span>
                <span className="text-xl font-bold leading-none">
                  {format(parseISO(selectedEvento.fecha_inicio), "d")}
                </span>
              </div>
              <div>
                <p className="font-semibold text-gray-900">{selectedEvento.titulo}</p>
                {selectedEvento.tipo && (
                  <p className="text-sm text-gray-500">{selectedEvento.tipo}</p>
                )}
                {selectedEvento.fecha_fin && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    Hasta{" "}
                    {format(parseISO(selectedEvento.fecha_fin), "d 'de' MMMM", { locale: es })}
                  </p>
                )}
              </div>
            </div>
              ...
              {selectedEvento.clientes && (
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {(selectedEvento.clientes as { nombre: string }).nombre}
                  </p>
                </div>
              )}
            </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div className="space-y-3">
            <div>
              ...
              <input
                {...register("titulo")}
                placeholder="Excursion al museo"
