...
import { MessageSquare, Search, Plus, CheckCircle, Clock, Mail, MessageCircle, Send, Loader2 } from "lucide-react"
  ...
  const columns = useMemo(() => [
    col.accessor("tipo", {
      ...
      cell: (info) => {
        ...
        const colorClass = v ? (TIPO_COLORS[v] ?? "bg-gray-50 text-gray-600") : "bg-gray-50 text-gray-600"
        return v
          ? <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colorClass}`}>{v}</span>
          : null
      },
    }),
    col.accessor("titulo", {
    ...
    col.accessor("clientes", {
      ...
      cell: (info) => {
        ...
        return <span className="text-sm text-gray-600">{c?.nombre ?? <span className="italic text-gray-300">Todos</span>}</span>
      },
    }),
    ...
    col.accessor("fecha_envio", {
      ...
      cell: (info) => {
        const v = info.getValue() as string | undefined
        return v
          ? <span className="text-xs text-gray-400">{format(new Date(v), "d MMM yyyy", { locale: es })}</span>
          : null
      },
    }),
    col.accessor("comunicado_id", {
      ...
      cell: (info) => {
        ...
        return (
          <button
            onClick={(e) => { e.stopPropagation(); marcarEnviado.mutate(info.getValue()) }}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            ...
            {borradores > 0 && <span className="ml-2 text-amber-500 font-medium">· {borradores} borradores</span>}
            {enviados > 0 && <span className="ml-2 text-emerald-500 font-medium">· {enviados} enviados</span>}
          </p>
        </div>
              ...
              <tbody>
                {table.getRowModel().rows.length === 0 ? (
                  ...
                  table.getRowModel().rows.map((row) => (
                    <tr key={row.id} onClick={() => setSelectedComunicado(row.original)}
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Fecha envio", value: selectedComunicado.fecha_envio ? format(new Date(selectedComunicado.fecha_envio), "d MMM yyyy HH:mm", { locale: es }) : undefined },
            ...
            {selectedComunicado.cuerpo && (
              <div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-4">
                  <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{selectedComunicado.cuerpo}</p>
                </div>
              </div>
            )}
            {!selectedComunicado.enviado && (
              <button
                onClick={() => { marcarEnviado.mutate(selectedComunicado.comunicado_id); setSelectedComunicado(null) }}
                disabled={marcarEnviado.isPending}
      ...
      <SlidePanel open={showCreate} onClose={() => setShowCreate(false)}
        title="Nuevo comunicado" subtitle="Crea y previsualiza antes de guardar" width="xl">
