...
function FaqCard({ faq }: { faq: Faq }) {
  ...
  const colorClass = faq.categoria
    ? (CAT_COLORS[faq.categoria] ?? "bg-gray-100 text-gray-600")
    : "bg-gray-100 text-gray-600"

  return (
    <div
      className={`bg-white border rounded-xl overflow-hidden transition-all ${faq.activa ? "border-gray-200" : "border-gray-100 opacity-60"}`}
    >
      <button
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
            ...
            <button
              onClick={() => {
                if (
                  faq.id &&
                  window.confirm("¿Eliminar esta FAQ? Esta acción no se puede deshacer.")
                )
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input
                {...register("pregunta")}
                placeholder="Como puedo cambiar el tamano del uniforme?"
              ...
              />
              {errors.pregunta && (
                <p className="text-xs text-red-500 mt-1">{errors.pregunta.message}</p>
              )}
            </div>
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-700 mb-1">
              ...
              />
              {errors.respuesta && (
                <p className="text-xs text-red-500 mt-1">{errors.respuesta.message}</p>
              )}
            </div>
            <div>
