...
export function AusenciasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (periodoFilter === "semana")
      d = d.filter((a) => isThisWeek(parseISO(a.fecha), { weekStartsOn: 1 }))
    ...
    () => [
        ...
        cell: (info) => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-sm font-medium ${today ? "text-orange-600" : "text-gray-900"}`}
              >
      ...
      col.accessor("alumnos", {
        ...
        cell: (info) => {
          ...
          return (
            <span className="text-sm text-gray-700 font-medium">{a?.nombre_completo ?? "-"}</span>
          )
        },
      }),
      col.accessor("motivo", {
        ...
        cell: (info) => (
          <span className="text-sm text-gray-500">
            {(info.getValue() as string | undefined) ?? (
              <span className="text-gray-300 italic">Sin especificar</span>
            )}
          </span>
        ),
      }),
        ...
        cell: (info) =>
          info.getValue() ? (
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-emerald-600">Si</span>
            </div>
          ) : (
            <div className="flex items-center gap-1">
              <XCircle size={14} className="text-orange-400" />
              <span className="text-xs text-orange-500">No</span>
            </div>
          ),
      }),
    ],
  ...
  const estaSemana = ausencias.filter((a) =>
    isThisWeek(parseISO(a.fecha), { weekStartsOn: 1 })
  ).length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          value={justificadaFilter}
          onChange={(e) => setJustificadaFilter(e.target.value)}
          className="text-sm border border-gray-200 rounded-lg px-3 py-2"
        >
